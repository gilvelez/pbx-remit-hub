...
exports.handler = async (event) => {
  try {
    ...
    if (usd < amount) return json(400, { error: "Insufficient funds" });

    const updated = await setWallet(db, session.userId, { usd: usd - amount });

    return json(200, { ok: true, wallet: { usd: updated.usd, php: updated.php } });

  } catch (err) {
    console.error("banks-withdraw error:", err);
