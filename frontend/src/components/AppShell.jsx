...
const HomeIcon = ({ active }) => (
  <svg className={`w-6 h-6 ${active ? 'text-[#0A2540]' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <button
            ...
            className="text-sm text-gray-500 hover:text-[#0A2540] transition"
              ...
              >
                {({ isActive }) => (
                  <>
                    ...
                    <span className={`text-xs mt-1 font-medium ${isActive ? "text-[#0A2540]" : "text-gray-400"}`}>
                      {label}
                    </span>
                  </>
                )}
