...
import { getQuote } from "../../lib/mockApi";

export default function Home() {
  const { session } = useSession();
  const navigate = useNavigate();
  const [rate, setRate] = useState(56.25);
  const [lastUpdated, setLastUpdated] = useState(new Date());

  // Simulate live rate updates
  useEffect(() => {
    const fetchRate = async () => {
      const quote = await getQuote(100);
      setRate(quote.rate);
    ...
    <div className="px-4 py-6">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-6">
        <div className="flex items-center justify-between mb-4">
          <span className="text-sm font-medium text-gray-500">Today's Rate</span>
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-3xl font-bold text-[#0A2540]">₱{rate.toFixed(2)}</span>
          <span className="text-gray-500">per $1 USD</span>
        </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500">No fees • Instant delivery</span>
        </div>
      </div>
